...
final class RunnerTests: XCTestCase {
  ...
  private func executeOnMain(command: Command) throws -> Response {
    if let requestedBundle = command.appBundleId, currentBundleId != requestedBundle {
      let target = XCUIApplication(bundleIdentifier: requestedBundle)
      NSLog("AGENT_DEVICE_RUNNER_ACTIVATE bundle=%@ state=%d", requestedBundle, target.state.rawValue)
      target.activate()
      currentApp = target
      currentBundleId = requestedBundle
    }
    let activeApp = currentApp ?? springboard
